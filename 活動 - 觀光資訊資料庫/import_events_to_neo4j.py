...
def clean_event(raw: dict) -> Optional[dict]:
    ...
    # Images 可能有多張，用平行陣列儲存；過濾掉空值
    images = raw.get("Images") or []
    image_names = [clean_str(i.get("Name")) for i in images if clean_str(i.get("Name"))]
    image_urls = [clean_str(i.get("URL")) for i in images if clean_str(i.get("URL"))]
    image_descriptions = [clean_str(i.get("Description")) for i in images if clean_str(i.get("Description"))]
...
# 如果你的機器比較舊、想換更輕量的模型，可以改成：
#   "paraphrase-multilingual-MiniLM-L12-v2"（384 維，速度快很多，但語意精準度較低）
...
SET
    ...
    e.imageNames = row.imageNames,
    e.imageUrls = row.imageUrls,
    e.imageDescriptions = row.imageDescriptions,
